...
START_DATE = dt.date.today() - dt.timedelta(days=30)  #'2021-08-17'
...
def plot_box(database, quantity):
    ...
    ax[1, 1].axis("off")
    table = ax[1, 1].table(
        cellText=cellText, colLabels=summary.columns, cellLoc="center", loc="best"
    )
...
def plot_density(database, quantity):
    ...
    for act_id in act_ids:
        ...
        running_intervals["distance"].plot.density(
            label=act_id.strftime("%Y-%m-%d"), ax=ax[0]
        )
        running_intervals["duration"].plot.density(
            label=act_id.strftime("%Y-%m-%d"), ax=ax[1]
        )
