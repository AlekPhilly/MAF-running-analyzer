...
def parse_garmin_tcx(filename):
    ...
    ns = {
        "ns0": "{http://www.garmin.com/xmlschemas/TrainingCenterDatabase/v2}",
    ...
    data = DataFrame(
        columns="time,distance,HR,speed,cadence,latitude,longitude,altitude".split(",")
    )

    for n, trackpoint in enumerate(trackpoints):
        data.loc[n, "time"] = trackpoint.find(".//" + ns["ns0"] + "Time").text
        data.loc[n, "distance"] = float(
            trackpoint.find(".//" + ns["ns0"] + "DistanceMeters").text
        )
        data.loc[n, "altitude"] = float(
            trackpoint.find(".//" + ns["ns0"] + "AltitudeMeters").text
        )
        data.loc[n, "HR"] = int(
            trackpoint.find(".//" + ns["ns0"] + "HeartRateBpm/").text
        )
        try:
            data.loc[n, "latitude"] = float(
                trackpoint.find(".//" + ns["ns0"] + "LatitudeDegrees").text
            )
        except:
            data.loc[n, "latitude"] = nan
        try:
            data.loc[n, "longitude"] = float(
                trackpoint.find(".//" + ns["ns0"] + "LongitudeDegrees").text
            )
        except:
            data.loc[n, "longitude"] = nan
        try:
            data.loc[n, "speed"] = float(
                trackpoint.find(".//" + ns["ns3"] + "Speed").text
            )
        except:
            data.loc[n, "speed"] = nan
        try:
            data.loc[n, "cadence"] = (
                int(trackpoint.find(".//" + ns["ns3"] + "RunCadence").text) * 2
            )
        except:
...
def clean_garmin_tcx_data(data):
    ...
    data[["speed", "cadence", "latitude", "longitude", "altitude"]] = data[
        ["speed", "cadence", "latitude", "longitude", "altitude"]
...
def extract_running_intervals(data):
    ...
    walk_cadence = min(
        [x for x in cadence if 0 < x < avg_cadence], key=lambda x: avg_cadence - x
    )
    ...
    for n in range(1, len(data.index)):
        if (
            data.loc[n, "cadence"] > walk_cadence
            and data.loc[n - 1, "cadence"] <= walk_cadence
        ):
    ...
    running_intervals = DataFrame(
        columns=["start_time", "stop_time", "start_dist", "stop_dist"]
    )
    ...
    for start, stop in zip(started_running, stopped_running):
        ...
        running_intervals = running_intervals.append(
            Series(
                {
    ...
    running_intervals["duration"] = (
        running_intervals["stop_time"] - running_intervals["start_time"]
    )
    running_intervals["distance"] = (
        running_intervals["stop_dist"] - running_intervals["start_dist"]
    )

    running_intervals["HRrate"] = abs(
        running_intervals["dHR"] / running_intervals["duration"] * 60
    )
